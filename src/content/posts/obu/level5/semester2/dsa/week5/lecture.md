...
- Binary Trees<br>二叉树
- Recursive techniques with trees<br>基于树的递归技术
- Traversals<br>遍历

## What is a Tree (Graph Theory)? <br>什么是树（图论）
...
## What is a Tree (Computer Science)? <br>什么是树（计算机科学）
...
## How do we represent binary trees? <br>如何表示二叉树呢？
...
## Represent Binary Trees of Integers <br>用整数表示二叉树
...
## Recursive Tree Algorithms <br>递归树算法
...
#### An implementation of pre-order traversal <br>先序遍历实现
...
#### An implementation of in-order traversal <br>中序遍历实现
...
#### An implementation of post-order traversal <br>后序遍历实现
...
## Requirements for this week <br>本周学习要求
...
# Binary search tree <br>二叉搜索树

## A Binary Search Tree <br>二叉搜索树示例
...
## Insertion into a Search Tree <br>插入到搜索树中
...
### Insertion into a Search Tree: Example <br>搜索树插入示例
...
## Searching a Search Tree <br> 搜索一个搜索树
...
## Special situation with binary search trees <br>二叉搜索树的特殊情况
...
## Best to make the class *contain* a tree, not *be* one <br>最好让类“包含”一棵树，而不是“本身就是”一棵树
...
### Recursive `addNode` <br>递归版 `addNode`
...
#### `BinarySearchTree.insert` using recursive `addNode` <br>使用递归 `addNode` 的 `BinarySearchTree.insert`
...
### Iterative `addNode` <br>循环版 `addNode`
...
#### `BinarySearchTree.insert` using iterative `addNode` <br>使用循环 `addNode` 的 `BinarySearchTree.insert`
...
## Perfectly balanced trees <br>完全平衡树
